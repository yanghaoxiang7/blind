...
namespace supp {
	...
	struct svec {
		...
		template<int lena, class fn>
		std::array<this_type, lena> bin(fn c) const {
...
namespace Complete_Simulator {
	...
	template<class T>
	inline T lowbit(T t) { return t & (-t); }
	inline int TankId(u8 t) { return (LTank >> (t >> 2)) & 3; }
	const u8 inilis[4] = {32u, 96u, 104u, 40u};
	struct pos { u8 x:4, y:4; operator bool() const; operator int() const; };
	...
	struct board {
		...
		int dangerous(int tankMe, int move, int tankOpp) {
			...
			if (move < 4) {
				...
				if(to == opp && canShot(tankOpp))return 2;
				...
				if (operator[](sh) == brick) {
					...
					//			cerr << "Here" << endl;
						...
						if (maha(opp, me) <= maha(sh, me))goto _l_safe;
		...
		void enemy_shoot(int x,int y,bool f){
			if((possible_position[0]&can_shoot[y][x])&&(possible_position[1]&can_shoot[y][x]))return;
				...
				if (!((dick >> i) & 1) && !move_move(r[i])) {
					...
					for (; in_range(x += tx) && in_range(y += ty);)
						if (int c = om[y][x] & stopping) {
							switch (om[y][x] & 7) {
								case 0:break;
								case brick:tox[i&1]=x;toy[i&1]=y;bb[y][x]=true;break;
								case steel: break;
								case headq: enemy_shoot(x,y,f);break;// return true;
								default: exit(1);
							}
							break;
						}
		...
		struct Map{
			...
			void Output(){
				for(int i=0;i<9;i++){
					for(int j=0;j<9;j++)printf("%d",(x[i/3]>>(i%3*9+j))&1);
					puts("");
				}
			}
			...
			void cant_be(int x0,int y0,int x1,int y1){
				...
				if(y0>y1)swap(y0,y1);
				if(x0==x1)
					for(int i=y0+1;i<y1;i++)x[i/3]&=134217727^(1<<(i%3*9+x0));
				else
					for(int i=x0+1;i<x1;i++)x[y0/3]&=134217727^(1<<(y0%3*9+i));
			}
			...
			for (int i = 0; i < e; ++i) {
				...
				for(int I=0;I<9;I++){
					for(int j=0;j<9;j++){
						...
						Map tmp;
						for(int k=I-1;k>=0;k--){
							if(((m[j][k]&7)==brick)||((m[j][k]&7)==steel))break;
							tmp.set(k,j);
						...
						for(int k=j-1;k>=0;k--){
							if(((m[k][I]&7)==brick)||((m[k][I]&7)==steel))break;
							tmp.set(I,k);
							can_shoot[j][I]=can_shoot[j][I]|tmp;
						}
						for(int k=j+1;k<9;k++){
				...
				if(i==e-1){
					if(ply){
						if((op[0]>0)&&(op[0]&4))Shoot[0]=1;
						if((op[1]>0)&&(op[1]&4))Shoot[1]=1;	
					}else{
						if((op[2]>0)&&(op[2]&4))Shoot[0]=1;
						if((op[3]>0)&&(op[3]&4))Shoot[1]=1;	
					}
				}
				if (!ply)swap(v1, v2);
				v1=c[i+1];
				...
				for(int j=0;j<v1["destroyed_blocks"].size();j+=2){
					...
					if(bb[y][x]==0)enemy_shoot(x,y,i==e-1);
					shooted[y][x]=true; 
					m[y][x]=0;
				}
				...
				for(int j=0;j<2;j++)
					if(tox[j]!=-1){
						...
						if(shooted[y][x]){
							if(ply){
								possible_position[0].cant_be(x,y,t[2]>>4,t[2]&15);
								possible_position[0].cant_be(x,y,t[3]>>4,t[3]&15);
							}else{
								possible_position[0].cant_be(x,y,t[0]>>4,t[0]&15);
								possible_position[0].cant_be(x,y,t[1]>>4,t[1]&15);
							}
						}else enemy_shoot(t[j+ply*2]>>4,t[j+ply*2]&15,i==e-1);
					}
...
#undef __
	...
	struct DistA {
		...
		pos sttt;
		board *ref;
		inline int fdis(int T,int t){return min(abs(T%9-(t>>4)),abs(T/9-(t&15)));}
		void operator()(board &gg, u8 start) {
			...
			if(start==4){
				...
				for(int i=0;i<81;i++)if(gg.possible_position[0].get(i%9,i/9)){
					...
					else if((mat[i]==mat[u])&&(fdis(i,gg.t[1])<fdis(u,gg.t[1])))u=i;
				}
				if(u!=-1){
					int x=u%9,y=u/9;
					gg.t[2]=(x<<4)|y;
				...
				for(int i=0;i<81;i++)if(gg.possible_position[1].get(i%9,i/9)){
					...
					else if((mat[i]==mat[u])&&(fdis(i,gg.t[0])<fdis(u,gg.t[0])))u=i;
				}
				if(u!=-1){
					int x=u%9,y=u/9;
					gg.t[3]=(x<<4)|y;
					gg.m[y][x]|=tanks[3];
					gg.dick&=255^8^128;
					if(gg.Shoot[1])gg.dick|=8;
					else gg.dick&=247;
				}
//				for(int i=0;i<9;i++){
//					for(int j=0;j<9;j++)printf("%d ",mat[i*9+j]);
//					puts("");
//				}
				_debug(gg.possible_position[0].Output();
				puts("");
				gg.possible_position[1].Output();
				gg.Output();
	...
	struct DistB {
		...
		void operator()(board &gg, const u8 start) {
			...
			while (ql != qr) {
				int c = que[ql++];
				int x = c % 9, y = c / 9, tds = mat[c] + 1 + (exdis[gg.m[y][x] & 7]);// + (gg.m[y][x] & tankmask ? 1 : 0);
				for (int i = 0; i < 4; ++i) {
					int tx = x + dx[i], ty = y + dy[i];
	...
	struct HeatMap {
		...
		struct BMap {
			...
			void set(const pos x) {
				if (x.y < 5) c1 |= 1ull << int(x);
				else c2 |= 1ull << ((int) x - 45);
			}
			template<class T>
			inline void forAll(T a) {
				u64 r1 = c1, r2 = c2;
				while (r1) {
		...
		HeatMap(board &glob, DistA &dist, const pos &starting) {
			...
			for (int i = dist[starting]; i >= 0; --i)
				distMap[i].forAll([&](pos c) {
					...
					updPos.map([&](const pos r) { return heat[(int) r] += heat[(int) c] / updPos.len; });
					if(dist.inAtkRange(c)) {
						const auto j=heat[(int)c];
						for (int i = glob.shotTo(c, makepos(4, 0)); !(makepos(4,0)==(c = movedir(c, i)));)
							heat[(int) c] =j;
					}
				});
		}
		void print(FILE *out = stderr) {
			for (int i = 0; i < 9; ++i)
				for (int j = 0; j < 9; ++j)
					fprintf(out, "%-4d%c", int(heat[i * 9 + j] / double(1u << 31) * 9999.999), j == 8 ? '\n' : ' ');
		}
	};
}
#include "cassert"
...
namespace Policy {
	...
	template<class T>
	typename T::content randOp(T c) {
		const int len = c.len;
		return c[unsigned_sample(rnpool, len)];
	}
	template<>
	opvec::content randOp<opvec>(opvec c) {
		const int len = c.len;
		if (len == 0) return -1;
		return c[unsigned_sample(rnpool, len)];
	}
	struct Walk {
		...
		opvec allMove(int tank, int letShoot = 1, int letStill = 1) {
			...
			for (int i = 0; i < 4; ++i)
				if (valid(t + dire[i])) {
					...
					if (!(glo->operator[](pp)) || glo->fore.get(pp.x,pp.y))ret.push(i);
		...
		i8 filterMinimum(int tank, opvec a) {
			...
			return randOp(a.minvec([dd, thi, this](int mov) {
				...
				return int(ar * 2) + (ar < dd ? mov <= 3 : mov > 3);
			}));
		}
		i8 operateSemi(int tank, int move) {
			...
			array<int, 4> cr1, cr2;
			switch (mainPivot) {
				case 1: if (canS)return select(3, cr1 = {0, 1, 5}, cr2 = {dire + 4, dire, -1});
		...
		i8 genMove(int tank) {
			...
			const int oppCh = ([&]() -> int {
				return tank ^ 3;
				/*return maha(thi, t1) < maha(thi, t2) ? 0 : (maha(thi, t1) == maha(thi, t2)
			...
			if (glo->canMove(tank^2) && !glo->canShot(tank ^ 2) && !allMove(tank ^ 2).filter([&](int mov) {
				return glo->dangerous(tank ^ 2, mov, tank) != 2;
			}))
				return glo->shotTo(thi, t1) + 4;
			if (glo->canMove(tank^3) && !glo->canShot(tank ^ 3) && !allMove(tank ^ 3).filter([&](int mov) {
			...
			const auto latk = [&]() -> i8 {
				...
				_debug(
				safe.map([&](int mov) {
					cerr << thi<<mov << "(:)" << (int) opph[thi] << "," << (int) opph.approx(thi, mov) <<endl;
					return 0;
				});)
				_log("Lb")
			...
			if (thi.y < 4 && opph[thi] > myh[oppT]) {
				if (!glo->canMove(tank^3))return latk();
				cerr << "Fa0" << endl;
				const opvec amv = allMove(oppTnk);
				const opvec atk = filterAttack(amv, oppTnk, myh);
				const auto fatk = atk.bin<3>([&](int mov) { return glo->dangerous(oppTnk, mov); });
				_debug(
				cerr << (int) fatk[0].len << " " << (int) fatk[1].len << " " << (int) fatk[2].len << " " << endl;
				cerr << (int) fatk[0][0] << endl;
				)
				const auto cutd = [&]() {
					if (glo->dangerous(tank, -1))
						if (!glo->canShot(tank)) return latk();
						else shootOpp(tank);
				...
				if (fatk[0]) {
					...
					if (thi == oppT) {
						if (glo->canShot(oppTnk)) {
							const i8 moveback = ([&]() {
								if (const opvec dirx = atk.filter([&](int mov) { return mov < 4; }))
									return randOp(dirx);
									...
									return i8(randOp(atk.filter([&](int mov) {
										return glo->operator[](movedir(oppT, mov ^ 6)) & (tanks[2] | tanks[3]);
									})) ^ 2);
								else return i8(-1);
							})();
							if (!glo->canShot(tank))
								if (rnpool->extrude_bit(1)) return -1;
								else return moveback;
							else return moveback;
						}
					} else if (const opvec cr = fatk[1].filter(
						[&](int mov) { return glo->dangerous(oppTnk, mov, tank) == 1; })) {
						...
						if (maha(thi, oppT) == 2 && allMove(oppTnk, 0, 0).filter(
							[r = myh[oppT], this, oppT](int mov) { return myh.approx(oppT, mov) == r; }))
							return -1;
						if (glo->canShot(tank))return rnpool->extrude_bit(2) ? shodir + 4 : md2;
						else return rnpool->extrude_bit(2) ? -1 : movdir;
					} else return cutd();
				} else if (glo->canShot(tank))
					if (int t = shootOpp(tank)) return t;
					else return -1;
